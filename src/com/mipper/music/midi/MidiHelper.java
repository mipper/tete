...
public class MidiHelper
{
  ...
  public ArrayList<Info> getAvailableSynthesizers ()
  {
    ArrayList<Info> synths = new ArrayList<Info> ();
    ...
    for ( Info info : infos )
    {
      try
      {
        ...
        if ( device instanceof Synthesizer )
        {
          ...
          Logger.debug ( String.format ( "Name: %s, Latency: %d, Voices: %d", info.getName(), synth.getLatency (), synth.getMaxPolyphony () ) );
        }
      }
      ...
      MidiException
  {
    ...
    _instruments = synth.getAvailableInstruments ();
    synth.close ();
    if ( _instruments == null || _instruments.length == 0 )
    {
